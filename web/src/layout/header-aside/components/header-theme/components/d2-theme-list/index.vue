<template>
  <el-table :data="list" v-bind="table">
    ...
    <el-table-column label="Aperçu" width="120">
      <div slot-scope="scope" class="theme-preview" :style="{ backgroundImage: `url(${$baseUrl}${scope.row.preview})` }"/>
    </el-table-column>
    <el-table-column prop="address" align="center">
      <template slot-scope="scope">
        <el-button v-if="activeName === scope.row.name" type="success" icon="el-icon-check" round>Actif</el-button>
        <el-button v-else round @click="handleSelectTheme(scope.row.name)">Appliquer</el-button>
      </template>
    </el-table-column>
  </el-table>
</template>
